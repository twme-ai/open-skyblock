...
2. Private Islands
   - World allocation, configurable world border, spawn, saved island home, named island warps, block-based Teleport Pad linking, guarded reset flow, visitor access toggles, island visiting, basic co-op invites/members, configurable co-op action permissions, island management UI, and protected interaction rules are implemented.
   - Fully shared cooperative profiles, per-member permission roles, and schematic placement remain future work.
