...
1. Foundation
   ...
   - Minion definitions, placeable minion items, persisted locations, generated storage, adjacent external storage blocks, configurable fuel, configurable upgrade slots, compactor-style output conversion, hopper-style overflow selling, offline ticking, claim menus, and pickup flow.

2. Private Islands
   - World allocation, configurable world border, spawn, saved island home, named island warps, guarded reset flow, visitor access toggles, island visiting, basic co-op invites/members, island management UI, and protected interaction rules are implemented.
   - Fully shared cooperative profiles, deeper permissions, block-based Teleport Pad linking, and schematic placement remain future work.
   - Minion placement blocks, adjacent external storage blocks, configurable fuel, configurable upgrade slots, compactor-style output conversion, hopper-style overflow selling, generated storage, offline generation, and collection credit are implemented; skins remain future work.

3. Menus and Recipes
   - Configurable SkyBlock menu, profile viewer, Quest Log, skill menu, paged recipe book, paged collection menu, bank menu, and minion UI are implemented.
   - Recipe registration, shaped recipes, collection-tier and Slayer-level crafting requirements, and unlock checks.
